...
class TestReadMetadata:
    @staticmethod
    def test_read_metadata_makes_metadata_df_with_the_expected_columns(path_to_resources):
        ...
        assert (abo_formatting.metadata_df.columns == expected_metadata_df.columns).all()


class TestTranslationToEn:
    cases_grid = [
        {"text": "multi-colored", "src_language": "en", "expected_text": "multicolored"},
    ...
    def test_translation_to_en_returns_expected_str(text, src_language, expected_text, path_to_resources):
        abo_formatting = ABOFormatting(path_to_resources)
